...
def broadcast_player_list():
    """Invia a TUTTI i client la lista aggiornata dei giocatori"""
    with players_lock:
        users_list = [
...
def broadcast_game_state(room, data):
    """Invia lo stato del gioco a entrambi i giocatori nella stanza"""
            ...
            # --- INVITI DIRETTI ---
                ...
                with players_lock:
                    if target_id in players_data:
                        inviter_conn = players_data[target_id]["conn"]
                
                if response == "decline":
                    log(f"[Invito] {player_id} rifiuta {target_id}")
                    if inviter_conn:
                        send_msg(inviter_conn, {"type": "invite_declined", "from": player_id})
                    ...
                    if inviter_conn:
                        ...
                        with players_lock:
                            if target_id in players_data:
                                inviter_status = players_data[target_id]["status"]
                        ...
                        if inviter_status in ["online", "waiting"]:
                            ...
                            # 1. Manda alla schermata di gioco
                            ...
                            # 2. FIX: Aspetta un attimo che i client carichino la grafica
                            time.sleep(0.5) 
                            
                            # 3. Ora invia lo stato per abilitare la X
                            initial_state = {
                                "board": new_room.board, 
                                "turn": "X", 
                                "result": None, 
                                "status": "running"
                            }
